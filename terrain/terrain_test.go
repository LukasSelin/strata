		...
		for y := range dst.Height {
			for x := range dst.Width {
				if v := dst.Data[dst.Index(x, y)]; v == v || dst.IsValid(x, y) {
					t.Errorf("%v: cell (%d, %d) = %g valid=%v, want NaN and invalid", sz, x, y, v, dst.IsValid(x, y))
				}
			}
		}
	}
				...
				for _, extra := range []int{0, 5} {
					for _, masked := range []bool{false, true} {
						for _, special := range []float64{0, 0.2} {
							id := fmt.Sprintf("%s w=%d h=%d extra=%d masked=%v special=%v", name, w, h, extra, masked, special)
							seed := uint64(w*1000 + h*10 + extra)
							a := newFixture(rand.New(rand.NewPCG(seed, 9)), w, h, extra, masked, special)
							b := newFixture(rand.New(rand.NewPCG(seed, 9)), w, h, extra, masked, special)
...
func TestValidityMatchesNaive(t *testing.T) {
	for _, w := range []int{3, 62, 63, 64, 65, 66, 129} {
		for _, extra := range []int{0, 1, 61} {
			...
			for _, out := range []raster.Float32Raster{f.out1, f.out2} {
				for y := range out.Height {
					for x := range out.Width {
						...
						if got := out.IsValid(x, y); got != want {
							t.Fatalf("w=%d extra=%d: cell (%d, %d) valid=%v, want %v", w, extra, x, y, got, want)
						}
					}
				}
			}
			for i := range len(f.parentOut.Valid) * 64 {
				if !inWindow[i] && raster.MaskGet(f.parentOut.Valid, i) != raster.MaskGet(before, i) {
					t.Fatalf("w=%d extra=%d: bit %d outside the outputs changed", w, extra, i)
				}
			}
		}
	}
...
func TestUnmaskedDEM(t *testing.T) {
	...
	Slope(plain, dem, SlopeOptions{CellSize: 1})
	if plain.Valid != nil {
