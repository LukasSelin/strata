...
| `rawio/` | against the previous build: how a chunked call writes its output file — through system calls, which the OS serialises within one file, or a shared mapping (`engine.CreateRawFile`), and behind the computation (write-behind) — on tmpfs, ext4 and NTFS, 1 and 12 workers, with a write probe that takes the computation away. Numbers in [`rawio/RESULTS.md`](rawio/RESULTS.md). Not part of the suite |
...
| `cog/` | against another program: the `cog` module's GeoTIFF/COG reader timed against GDAL reading the same COGs (Deflate, ZSTD, LZW, none; 1 and 12 threads), slope over a COG against `gdaldem slope` and strata's raw path, and the block cache under halos. Its own Go module, since the core cannot import `cog`. Numbers in [`cog/RESULTS.md`](cog/RESULTS.md). Not part of the suite |
| `zarr/` | the `zarr` module's decoded-chunk cache: slope and an 11 × 11 focal mean over a Zarr source with the cache on and off, at chunks of 256² and 512², on 1 and all workers, and single-window reads cached and not. Its own Go module, like `cog/`. Numbers in [`zarr/RESULTS.md`](zarr/RESULTS.md). Not part of the suite |
| `gdalsuite/` | against another program, all of it: every operation with a GDAL counterpart (25: terrain, focal, algebra, reduce, resample) timed against GDAL at three levels — compute only (both tools' algorithms on a raster already in memory), file to file, and the whole flow from a Deflate COG — on 1 and 12 threads, with both tools' outputs compared on every operation. `BASELINE=` an earlier run gives a "since" table. Its own Go module, like `cog/`. Numbers in [`gdalsuite/RESULTS.md`](gdalsuite/RESULTS.md). Not part of the suite |
