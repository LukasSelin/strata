...
#   VARIANTS="old new"   binaries <BINDIR>/suite-<variant><EXE>
...
for r in $(seq 1 "$ROUNDS"); do
  for d in $DIRS; do for op in $OPS; do for s in $SRCS; do for n in $NS; do for v in $VARIANTS; do
    f=$d/dem.raw; [[ $s == cog ]] && f=$d/dem-cog.tif
    [[ -z ${NOSYNC:-} && $(stat -f -c %T "$d" 2>/dev/null) != tmpfs ]] && sync
    out=$(GOMAXPROCS=${GMP:-$n} "$BINDIR/suite-$v$EXE" -mode chunked -op "$op" -src "$s" -file "$f" \
      -w 11264 -h 11264 -fill 65535 -cell 12.5 -tile 256 -workers "$n" -dir "$d" 2>&1)
