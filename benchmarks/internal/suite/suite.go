...
//
// Only workers=1 exists today. Tiled multi-worker execution belongs to the
// engine (STRATA-8/9); when it lands, Workers grows to 1, physical cores
// and logical cores, and workloads receive the count through Case.
package suite
...
// Workers returns the worker counts to run. It is only 1 until the tile
// engine exists (STRATA-8/9).
func Workers() []int { return []int{1} }
...
func (c Case) Name() string {
	return fmt.Sprintf("size=%d/mask=%s/backend=%s/workers=%d", c.Size, onOff(c.Masked), c.Backend, c.Workers)
}
...
type Workload struct {
	// Run performs the operation once over the whole raster.
	Run func()
