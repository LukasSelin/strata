...
// over at most window² cells starting a third of the way in, and at most
// maxBands bands are compared.
const (
	maxCells = 1 << 24
	window   = 4096
	maxBands = 16
...
func levelWindow(w, h int) [4]int {
	...
	x, y := w/3, h/3
	return [4]int{x, y, min(w-x, window), min(h-y, window)}
}
...
// readAll reads the region win (x, y, width, height) of the source in
// 100×77 windows and returns its cells as little-endian float32 bytes and
// its validity as one byte per cell.
func readAll(src *cog.Source, win [4]int) (vals, mask []byte, err error) {
	x0, y0, w, h := win[0], win[1], win[2], win[3]
	...
	for y := 0; y < h; y += wh {
		for x := 0; x < w; x += ww {
			...
			if err := src.ReadWindow(context.Background(), win, x0+x, y0+y); err != nil {
				return nil, nil, err
			}
