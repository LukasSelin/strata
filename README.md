...
| `terrain` | Terrain derivatives from Horn's 3×3 gradient: `Gradient`, `Slope`, `Aspect`, `Hillshade`; profile, plan and mean `Curvature` from the Zevenbergen–Thorne quadratic; and `Ruggedness` (TRI, Riley or Wilson; TPI; roughness), bit-identical to gdaldem's; `HeatLoad`, McCune and Keon's heat load index and potential direct incident radiation; `Orientation`, northness and eastness. |
