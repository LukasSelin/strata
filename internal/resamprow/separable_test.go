...
// randomAxes returns the axes of a resampling over a random source of up
// to 48×48 cells at a random scale and offset, and its source size.
func randomAxes(rng *rand.Rand, m Method) (a *Axes, sw, sh int) {
...
func separable(p *Axes, win bool, src []float32, sw, sh int) []float32 {
	w, h := len(p.X.First), len(p.Y.First)
	out := make([]float32, w*h)
	if p.X.Lo >= p.X.Hi || p.Y.Lo >= p.Y.Hi {
		return out
	}
	fx0, fy0, fx1, fy1 := p.Footprint(p.X.Lo, p.Y.Lo, p.X.Hi, p.Y.Hi, win)
	cw := p.X.Hi - p.X.Lo
...
func TestSeparableIsDirect(t *testing.T) {
	...
	for iter := range 500 {
		for _, m := range []Method{Bilinear, Cubic, Lanczos, Average} {
			...
			got := separable(p, m == Lanczos, src, sw, sh)
			w, h := len(p.X.First), len(p.Y.First)
