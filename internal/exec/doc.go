...
// computes validity masks and checks cancellation, so that kernels only
// compute values (DESIGN.md §22, §23, §25). It runs on the calling
// goroutine; tiles on a worker pool are STRATA-9.
//
...
//
// engine.Options.TileWidth and TileHeight split the raster into tiles,
// processed in row-major order; Workers is validated and otherwise
// ignored until STRATA-9. Options never change the result, only the order
// in which cells are written.
//
...
// before the call writes nothing. Only cancellation is reported as an
// error; programming errors panic, like package raster.
//
// # Allocation
//
...
// # Concurrency
//
// Neither Process nor any kernel in this module starts goroutines.
