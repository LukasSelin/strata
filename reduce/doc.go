...
// largest of its valid cells (DESIGN.md §49). It is the other half of a
...
//	mn, mx, n, err := reduce.MinMaxChunked(ctx, demSource, engine.Options{TileHeight: 256})
//
// The operations are named, not a per-cell callback, for the reasons of
// DESIGN.md §19. Sum, Mean and Stats follow; histograms and quantiles are
// a later operation, because they return a vector rather than a scalar
...
//
// Sum will need more than this: floating-point addition is not
// associative, so an order-independent sum needs an accumulator that does
// not round as it goes (DESIGN.md §49).
//
